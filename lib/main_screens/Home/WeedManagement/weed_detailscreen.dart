...
class WeedDetailscreen extends StatelessWidget {
  final Weed weed;
  const WeedDetailscreen({super.key, required this.weed});
        ...
        slivers: [
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: Image.asset(weed.image_path, fit: BoxFit.cover),
            ),
          ),
          SliverPersistentHeader(
            delegate: _SliverTabBarDelegate(weed.title),
            pinned: true,
          ),
          if (weed.details.isNotEmpty)
            SliverList(
              delegate: SliverChildBuilderDelegate((context, index) {
                final item = weed.details[index];
                return DetailSection(item: item);
              }, childCount: weed.details.length),
            )
          else
            SliverFillRemaining(
              ...
              child: Center(
                child: Padding(
                  ...
                  child: TranslatedText(
                    "No details available for ${weed.title}.",
